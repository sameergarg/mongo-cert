...
def show_post(permalink="notfound"):
    ...
    post = db.posts.find_one({"title":permalink})
...
def post_newcomment():
    ...
    post = posts.find_one({"title":permalink})  # Replace this line with your work.
    ...
    if (name == "" or body == ""):
        ...
        try:
            ...
            # your update here.
            # posts.update({"title" : permalink},{"$push" : {"comments" : comment}})
            print("post id is "+post['_id'])
            posts.update({"id" : post['_id']},{"$push" : {"comments" : comment}})
